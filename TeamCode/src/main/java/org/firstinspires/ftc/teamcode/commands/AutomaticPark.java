...
public class AutomaticPark extends Command {

    private final DriveSubsystem drive;
    private final FlashLights flashLights;
    private final Gamepad gamepad;

    public AutomaticPark(DriveSubsystem drive, Lights lights, Gamepad gamepad) {
        this.drive = drive;
    ...
    public void initialize() {
        ...
        drive.getBase().setFollowPath(Path.getBuilder().setDefaultRadius(DriveConstants.defaultFollowRadius)
                .addWaypoint(botPose.x, botPose.y).addWaypoint(Tele.parkPose.toWaypoint()).build());
